...
extern std::unordered_map<uint512Index, uint256, Hash512Hasher>	g_oHashCatchMap;
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitcoinfile/bitcoinfile>";
    const std::string URL_WEBSITE = "<https://www.bitcoinfile.org>";
...
void ReadHashFromDesk( void ) {
	...
	if ( lpFile ){
		...
		{
			size_t liPos = 0;
			char* lszBuf = new char[liSize+1];
			if ( fread(lszBuf, 1, liSize, lpFile) == liSize )
			{
				while (( liPos + 98			<= liSize ) && 
					   ( lszBuf[liPos]		== lcPart ) && 
	...
	printf("[BIFI] Beg ReadHashFromDesk Cnt:%d\n", liCnt);
}

bool CreateDir(std::string& astrDir)
{
	if( (access( astrDir.c_str(), 0 )) == -1 ) 
#ifdef WIN32
		mkdir(astrDir.c_str());
#else
		mkdir(astrDir.c_str(), 0777);
#endif
	if((access( astrDir.c_str(), 0 )) != -1 )
		return true;
	return false;
...
{
	fs::path loHashFolderRoot = GetDataDir()/"hashindex/";
	g_sHashFolderRoot = loHashFolderRoot.string();
	CreateDir( g_sHashFolderRoot );
	ReadHashFromDesk();
