...
void WalletTxToJSON(const CWalletTx& wtx, UniValue& entry)
{
    ...
    if (wtx.tx->nVersion == CTransaction::CURRENT_VERSION_II)
        entry.push_back(Pair("fmk", wtx.tx->m_iForkMark ));
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
	...
	UniValue loAddrArray(UniValue::VARR);
	for (int liCnt = 0; liCnt <  nCount; ++liCnt){
		{
			if (!pwallet->IsLocked()) {
			...
			std::string lstrAddr = CBitcoinAddress(keyID).ToString();
			loAddrArray.push_back(lstrAddr);
			FILE* lpFile = fopen("address.txt","a+");
			if (lpFile){
				fwrite("\"", 1, 1, lpFile);
				fwrite(lstrAddr.c_str(), 1, lstrAddr.length(), lpFile);
				fwrite("\"\n", 1, 2, lpFile);
				fclose( lpFile );
			}
			//LogPrintf("[BIFI] getnewaddress Addr:%s finish:%d total:%d\n", lstrAddr, liCnt, nCount );
		}
	}
	return loAddrArray;
}
...
{ //  category              name                        actor (function)           okSafeMode
    ...
    { "wallet",             "getnewaddress",            &getnewaddress,            true,   {"account", "count"} },
