...
UniValue importprunedfunds(const JSONRPCRequest& request)
{
    ...
    if (!DecodeHexTx(tx, request.params[0].get_str()))
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "TX decode failed5");
	...
	InitPervInfo(&merkleBlock.header);
    if (merkleBlock.txn.ExtractMatches(vMatch, vIndex) == merkleBlock.header.hashMerkleRoot) {

        LOCK(cs_main);

		if (!mapBlockIndex.count(merkleBlock.header.GetHashImp( )) || !chainActive.Contains(mapBlockIndex[merkleBlock.header.GetHashImp( )]))
	...
	InitPervInfo(&merkleBlock.header);
    wtx.nIndex = txnIndex;
	wtx.hashBlock = merkleBlock.header.GetHashImp( );
    LOCK2(cs_main, pwallet->cs_wallet);
