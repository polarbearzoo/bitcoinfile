...
void PeerLogicValidation::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) {
	...
	InitPervInfo((CBlockHeader*)pblock.get());
...
void PeerLogicValidation::BlockChecked(const CBlock& block, const CValidationState& state) {
	...
	InitPervInfo((CBlockHeader*)(&block));
        ...
        const CInv &inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK || inv.type == MSG_CMPCT_BLOCK || inv.type == MSG_WITNESS_BLOCK)
            {
                ...
                if (send && (mi->second->nStatus & BLOCK_HAVE_DATA))
                {
					...
					if (a_recent_block) InitPervInfo((CBlockHeader*)a_recent_block.get());
                    ...
                    else if (inv.type == MSG_CMPCT_BLOCK)
                    {
						...
						if (a_recent_compact_block) InitPervInfo((CBlockHeader*)&a_recent_compact_block->header);
...
bool static ProcessHeadersMessage(CNode *pfrom, CConnman *connman, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool punish_duplicate_invalid)
{
    ...
    const CBlockIndex *pindexLast = nullptr;
    {
        ...
        for (const CBlockHeader& header : headers) {
			...
			InitPervInfo((CBlockHeader*)&header);
    ...
    if (!ProcessNewBlockHeaders(headers, state, chainparams, &pindexLast, &first_invalid_header)) {
        ...
        if (state.IsInvalid(nDoS)) {
			...
			InitPervInfo(&first_invalid_header);
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
		...
		InitPervInfo(&cmpctblock.header);
        ...
        //
        {
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.fPreferHeaderAndIDs) {
                    ...
                    //LogPrint(BCLog::NET, "%s sending header-and-ids %s to peer=%d\n", __func__,
                    //        vHeaders.front().GetHashIndex.ToString(), pto->GetId());
					//vHeaders.front().GetHashCheck().ToString(), pto->GetId());
